<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="model.user.User" import="model.train.Train" import="model.order.Order"%>
<%
					...
					<div class="row">
						<div class="span10">
							<div class="slate">
								...
								<form action="buyTicketSuccessful" method="post">
								<input type="hidden" name="uid" value=<%= user.getUid() %> />
								<input type="hidden" name="oid" value=<%= order.getOid() %> />
									<table class="orders-table table">
										<tbody>
											<tr>
												<td>您的座位已锁定：从从<%=startCity%>到<%=endCity%>的<%=train.getTid()%>车次,第<%=cid%>车厢<%=location%>座</td>
											</tr>
											<tr>
												<td><input type="submit" name="submit" value="付款" /></td>
											</tr>
										</tbody>
									</table>
								</form>
